...
export function CTAButtons({
    ...
    return (
        <div className={`flex flex-col sm:flex-row gap-4 ${className}`}>
            <a
                href={`tel:${formatPhoneLink(businessConfig.phone)}`}
                className="group relative inline-flex items-center justify-center px-8 py-4 text-lg font-semibold text-button-700 bg-white rounded-xl shadow-xl hover:shadow-2xl hover:bg-gray-50 transition-all duration-300 transform hover:scale-105 border-2 border-white"
            >
                <svg
                    className="w-5 h-5 mr-2 group-hover:rotate-12 transition-transform"
                    fill="none"
